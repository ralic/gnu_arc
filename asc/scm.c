...
void init_banner()
{
  fputs("ASC version "SCMVERSION", Copyright (C) 2002, 2003 \
Gregor Klinke.\n\
ASC comes with ABSOLUTELY NO WARRANTY; for details type `(terms)'.\n\
...
#  ifndef MSDOS
  if (script_arg) 
    exepath = script_find_executable(script_arg);
#  endif
# endif
     ...
     const char *command;
{
  ...
  if (pid) {
    ...
    sts = execl("/bin/rc", "/bin/rc", "-c", command, nil);
  return sts;
}
#endif
